...
if __name__ == "__main__":
    ...
    # Log hyperparameters
    logger.info("Training REINFORCE while playing against BigMoney")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=g.NUM_GAMES_PARALLEL) as executor:
        ...
        while True:
            ...
            for future in completed:
                try:
                    ...
                    if new_experience is not None:
                        ...
                        writer.add_scalar("player_reward", new_experience.end_reward, game_counter)
            ...
            if game_counter >= next_train:
                ...
                states, actions, turns, end_turns, end_rewards = safe_exp_buffer.get_batch()
                loss, entropy = train_policy(
                    g=g,
